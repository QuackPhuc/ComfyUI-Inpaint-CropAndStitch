...
class ProcessorLogic(ABC):
    ...
    def calculate_fisheye_rotation_angle(self, mask, image_h, image_w):
        """
        Calculate rotation angle based on mask centroid position relative to image center.
        For fisheye images from top-down view, objects should point toward center.
        Returns angle in degrees: 0, 90, 180, or 270.
        """
        # Find mask centroid
        B, H, W = mask.shape
        device = mask.device

        # Calculate centroid for the first mask in batch
        mask_binary = (mask[0] > 0.5).float()
        total = mask_binary.sum()

        if total == 0:
            return 0  # No mask, no rotation needed
        ...
        centroid_y = (y_coords * mask_binary).sum() / total
        centroid_x = (x_coords * mask_binary).sum() / total

        # Image center
        center_y = image_h / 2.0
        center_x = image_w / 2.0

        # Calculate angle from center to mask centroid
        dy = centroid_y.item() - center_y
        dx = centroid_x.item() - center_x

        # Convert to angle and quantize to 0, 90, 180, 270
        angle_rad = math.atan2(dy, dx)
        angle_deg = math.degrees(angle_rad)

        # Quantize:
        # -45 to 45 -> 0 (right side, rotate 90 CCW to make it point down)
        # 45 to 135 -> 90 (bottom side, rotate 180 to make it point down)
        # 135 to -135 -> 180 (left side, rotate 270 CCW to make it point down)
        # -135 to -45 -> 270 (top side, no rotation needed)
        if -45 <= angle_deg < 45:
            return 90  # Right side -> rotate 90 CCW
        elif 45 <= angle_deg < 135:
            return 180  # Bottom side -> rotate 180
        elif angle_deg >= 135 or angle_deg < -135:
            return 270  # Left side -> rotate 270 CCW (or 90 CW)
        else:  # -135 <= angle_deg < -45
            return 0  # Top side -> no rotation needed
...
class CPUProcessorLogic(ProcessorLogic):
    def rescale_i(self, samples, width, height, algorithm: str):
        ...
        for i in range(samples.shape[0]):
            samples_pil: Image.Image = F.to_pil_image(samples[i].cpu()).resize(
                (width, height), algorithm_enum
            )
    ...
    def expand_m(self, mask, pixels):
        ...
        for i in range(mask.shape[0]):
            ...
            results.append(
                torch.from_numpy(dilated_mask.astype(np.float32)).clamp(0.0, 1.0)
            )
        return torch.stack(results, dim=0)
    ...
    def preresize_imm(
        ...
        if preresize_mode == "ensure minimum resolution":
            if (
                current_width >= preresize_min_width
                and current_height >= preresize_min_height
            ):
            ...
            image = self.rescale_i(
                image, target_width, target_height, upscale_algorithm
            )
            mask = self.rescale_m(mask, target_width, target_height, "bilinear")
            optional_context_mask = self.rescale_m(
                optional_context_mask, target_width, target_height, "bilinear"
            )

            assert (
                target_width >= preresize_min_width
                and target_height >= preresize_min_height
            ...
            if (
                preresize_min_width <= current_width <= preresize_max_width
                and preresize_min_height <= current_height <= preresize_max_height
            ):
                return image, mask, optional_context_mask
            ...
            if scale_factor_min > 1 and scale_factor_max < 1:
                assert (
                    False
                ), "Cannot meet both minimum and maximum resolution requirements with aspect ratio preservation."

            if scale_factor_min > 1:  # We're upscaling to meet min resolution
                scale_factor = scale_factor_min
                rescale_algorithm = (
                    upscale_algorithm  # Use upscale algorithm for min resolution
                )
            else:  # We're downscaling to meet max resolution
                scale_factor = scale_factor_max
                rescale_algorithm = (
                    downscale_algorithm  # Use downscale algorithm for max resolution
                )
            ...
            image = self.rescale_i(
                image, target_width, target_height, rescale_algorithm
            )
            mask = self.rescale_m(
                mask, target_width, target_height, "nearest"
            )  # Always nearest for efficiency
            optional_context_mask = self.rescale_m(
                optional_context_mask, target_width, target_height, "nearest"
            )  # Always nearest for efficiency

            assert (
                preresize_min_width <= target_width <= preresize_max_width
            ), f"Internal error: Target width {target_width} is outside the range {preresize_min_width} - {preresize_max_width}"
            assert (
                preresize_min_height <= target_height <= preresize_max_height
            ...
            if (
                current_width <= preresize_max_width
                and current_height <= preresize_max_height
            ):
                return image, mask, optional_context_mask
            ...
            image = self.rescale_i(
                image, target_width, target_height, downscale_algorithm
            )
            mask = self.rescale_m(
                mask, target_width, target_height, "nearest"
            )  # Always nearest for efficiency
            ...
            assert (
                target_width <= preresize_max_width
                and target_height <= preresize_max_height
    ...
    def extend_imm(
        ...
        assert (
            new_H >= 0
        ), f"Error: Trying to crop too much, height ({new_H}) must be >= 0"
        assert (
            new_W >= 0
        ), f"Error: Trying to crop too much, width ({new_W}) must be >= 0"
        ...
        expanded_optional_context_mask = torch.zeros(
            B, new_H, new_W, device=optional_context_mask.device
        )
        ...
        expanded_image = expanded_image.permute(
            0, 3, 1, 2
        )  # [B, H, W, C] -> [B, C, H, W]
        ...
        expanded_image[
            :,
            :,
            slice_target_up:slice_target_down,
            slice_target_left:slice_target_right,
        ] = image[
            :,
            :,
            slice_source_up:slice_source_down,
            slice_source_left:slice_source_right,
        ]
        if up_padding > 0:
            expanded_image[:, :, :up_padding, slice_target_left:slice_target_right] = (
                image[:, :, 0:1, slice_source_left:slice_source_right].repeat(
                    1, 1, up_padding, 1
                )
            )
        if down_padding > 0:
            expanded_image[
                :, :, -down_padding:, slice_target_left:slice_target_right
            ] = image[:, :, -1:, slice_source_left:slice_source_right].repeat(
                1, 1, down_padding, 1
            )
        if left_padding > 0:
            expanded_image[:, :, slice_target_up:slice_target_down, :left_padding] = (
                expanded_image[
                    :,
                    :,
                    slice_target_up:slice_target_down,
        ...
        if right_padding > 0:
            expanded_image[:, :, slice_target_up:slice_target_down, -right_padding:] = (
                expanded_image[
                    :,
                    :,
                    slice_target_up:slice_target_down,
    ...
    def crop_magic_im(
        ...
        expanded_image[
            ...
            left_padding : left_padding + image_w,
        ] = image
        ...
        if up_padding > 0:
            expanded_image[:, :, :up_padding, left_padding : left_padding + image_w] = (
                expanded_image[
                    :,
                    :,
                    up_padding : up_padding + 1,
                    left_padding : left_padding + image_w,
                ].repeat(1, 1, up_padding, 1)
            )
        if down_padding > 0:
            expanded_image[
                :, :, -down_padding:, left_padding : left_padding + image_w
            ] = expanded_image[
                :,
                :,
                up_padding + image_h - 1 : up_padding + image_h,
                left_padding : left_padding + image_w,
            ].repeat(
                1, 1, down_padding, 1
            )
        ...
        if resize_output:
            ...
            if target_w > ctc_w or target_h > ctc_h:  # Upscaling
                cropped_image = self.rescale_i(
                    cropped_image, target_w, target_h, upscale_algorithm
                )
                cropped_mask = self.rescale_m(
                    cropped_mask, target_w, target_h, upscale_algorithm
                )
            else:  # Downscaling
                cropped_image = self.rescale_i(
                ...
                cropped_mask = self.rescale_m(
                    cropped_mask, target_w, target_h, downscale_algorithm
                )
    ...
    def rotate_image(self, image, angle):
        ...
        results = []
        for i in range(B):
            img_pil = F.to_pil_image(image[i].permute(2, 0, 1).cpu())  # [C, H, W]
            if angle == 90:
            ...
            results.append(F.to_tensor(img_pil).permute(1, 2, 0))  # Back to [H, W, C]
    ...
    def rotate_mask(self, mask, angle):
        ...
        results = []
        for i in range(B):
            mask_pil = F.to_pil_image(mask[i].cpu().unsqueeze(0))  # [1, H, W]
            if angle == 90:
            ...
            results.append(F.to_tensor(mask_pil).squeeze(0))  # Back to [H, W]
    ...
    def stitch_magic_im(
        ...
        if ctc_w > w or ctc_h > h:  # Upscaling
            resized_image = self.rescale_i(
                inpainted_image, ctc_w, ctc_h, upscale_algorithm
            )
            resized_mask = self.rescale_m(mask, ctc_w, ctc_h, upscale_algorithm)
        else:  # Downscaling
            resized_image = self.rescale_i(
                inpainted_image, ctc_w, ctc_h, downscale_algorithm
            )
...
class GPUProcessorLogic(ProcessorLogic):
    ...
    def expand_m(self, mask, pixels):
        ...
        dilated = TF.max_pool2d(
            mask_in, kernel_size=kernel_size, stride=1, padding=padding
        )
    ...
    def batched_findcontextarea_m(self, mask):
        ...
        def get_min_max(any_dim, size):
            ...
            min_indices = torch.where(
                any_dim, indices, torch.tensor(size, device=device)
            )
...
class InpaintCropImproved:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "preresize_min_width": (
                    "INT",
                    {"default": 1024, "min": 0, "max": nodes.MAX_RESOLUTION, "step": 1},
                ),
                "preresize_min_height": (
                    "INT",
                    {"default": 1024, "min": 0, "max": nodes.MAX_RESOLUTION, "step": 1},
                ...
                "extend_for_outpainting": (
                    "BOOLEAN",
                    {"default": False, "tooltip": "Extend the image for outpainting."},
                ),
                "extend_up_factor": (
                    "FLOAT",
                ...
                "output_target_width": (
                    "INT",
                    {"default": 512, "min": 64, "max": nodes.MAX_RESOLUTION, "step": 1},
                ),
    ...
    def inpaint_crop(
        ...
        if preresize and preresize_mode == "ensure minimum and maximum resolution":
            assert (
                preresize_max_width >= preresize_min_width
            ), "Preresize maximum width must be greater than or equal to minimum width"
            assert (
                preresize_max_height >= preresize_min_height
            ), "Preresize maximum height must be greater than or equal to minimum height"
        ...
        if image.shape[0] > 1:
            assert (
                output_resize_to_target_size
            ), "output_resize_to_target_size must be enabled when input is a batch of images, given all images in the batch output have to be the same size"
        ...
        if mask is not None and (
            ...
            if mask.shape[1] != image.shape[1] or mask.shape[2] != image.shape[2]:
                if torch.count_nonzero(mask) == 0:
                    mask = torch.zeros(
                        (mask.shape[0], image.shape[1], image.shape[2]),
                        device=image.device,
                        dtype=image.dtype,
                    )
        ...
        assert (
            optional_context_mask.shape[0] == image.shape[0]
        ...
        for i in range(batch_size):
            ...
            if preresize:
                sub_image, sub_mask, sub_opt_mask = processor.preresize_imm(
                    sub_image,
                    sub_mask,
                    sub_opt_mask,
                    downscale_algorithm,
            ...
            if mask_hipass_filter >= 0.01:
                ...
                sub_opt_mask = processor.hipassfilter_m(
                    sub_opt_mask, mask_hipass_filter
                )
            sub_DEBUG_hipassfilter_mask = sub_mask.clone() if self.DEBUG_MODE else None

            if extend_for_outpainting:
                sub_image, sub_mask, sub_opt_mask = processor.extend_imm(
                    sub_image,
                    sub_mask,
                    sub_opt_mask,
                    extend_up_factor,
            ...
            if context_from_mask_extend_factor >= 1.01:
                _, bx, by, bw, bh = processor.batched_growcontextarea_m(
                    sub_mask, bx, by, bw, bh, context_from_mask_extend_factor
                )

            # Combine
            _, bx, by, bw, bh = processor.batched_combinecontextmask_m(
                sub_mask, bx, by, bw, bh, sub_opt_mask
            )
            ...
            # Fisheye rotation: calculate and apply rotation to cropped region
            rotation_angle = 0
            if fisheye_rotation:
                # Use the processed mask (sub_mask) and original image dimensions to calculate angle
                rotation_angle = processor.calculate_fisheye_rotation_angle(
                    sub_mask, sub_image.shape[1], sub_image.shape[2]
                )
                if rotation_angle != 0:
                    p_crop = processor.rotate_image(p_crop, rotation_angle)
                    p_mask = processor.rotate_mask(p_mask, rotation_angle)
            ...
            result_stitcher["rotation_angle"].append(rotation_angle)
            ...
            if self.DEBUG_MODE:
                ...
                co = (
                    ...
                    cur_w,
                    cur_h,
                )  # This is combined coordinates actually, need stages if we want them.
                # However, processing is 1 by 1 now, so we can just track them.

                # Re-calculate stages for individual debug accuracy
                _, b_orig_x, b_orig_y, b_orig_w, b_orig_h = (
                    processor.batched_findcontextarea_m(sub_mask)
                )
                if b_orig_x[0] == -1:
                    b_orig_x[0], b_orig_y[0], b_orig_w[0], b_orig_h[0] = (
                        0,
                        0,
                        sub_image.shape[2],
                        sub_image.shape[1],
                ...
                b_grown_x, b_grown_y, b_grown_w, b_grown_h = (
                    b_orig_x.clone(),
                    b_orig_y.clone(),
                    b_orig_w.clone(),
                    b_orig_h.clone(),
                ...
                if context_from_mask_extend_factor >= 1.01:
                    _, b_grown_x, b_grown_y, b_grown_w, b_grown_h = (
                        processor.batched_growcontextarea_m(
                            sub_mask,
                            b_orig_x,
                            b_orig_y,
                            b_orig_w,
                ...
                b_comb_x, b_comb_y, b_comb_w, b_comb_h = (
                    b_grown_x.clone(),
                    b_grown_y.clone(),
                    b_grown_w.clone(),
                    b_grown_h.clone(),
                )
                _, b_comb_x, b_comb_y, b_comb_w, b_comb_h = (
                    processor.batched_combinecontextmask_m(
                        sub_mask,
                        b_grown_x,
                        b_grown_y,
                        b_grown_w,
                ...
                def get_debug_crop_cpu(m, c):
                    ...
                    if output_resize_to_target_size and (
                        crop.shape[2] != output_target_width
                        or crop.shape[1] != output_target_height
                    ):
                        if isinstance(processor, GPUProcessorLogic):
                            crop = processor.rescale_m(
                                crop,
                                output_target_width,
                                output_target_height,
                                "nearest",
                ...
                debug_outputs["DEBUG_preresize_image"].append(
                    sub_DEBUG_preresize_image[0].cpu()
                )
                debug_outputs["DEBUG_preresize_mask"].append(
                    sub_DEBUG_preresize_mask[0].cpu()
                )
                debug_outputs["DEBUG_fillholes_mask"].append(
                    sub_DEBUG_fillholes_mask[0].cpu()
                )
                debug_outputs["DEBUG_expand_mask"].append(
                    sub_DEBUG_expand_mask[0].cpu()
                )
                debug_outputs["DEBUG_invert_mask"].append(
                    sub_DEBUG_invert_mask[0].cpu()
                )
                debug_outputs["DEBUG_blur_mask"].append(sub_DEBUG_blur_mask[0].cpu())
                debug_outputs["DEBUG_hipassfilter_mask"].append(
                    sub_DEBUG_hipassfilter_mask[0].cpu()
                )
                debug_outputs["DEBUG_extend_image"].append(
                    sub_DEBUG_extend_image[0].cpu()
                )
                debug_outputs["DEBUG_extend_mask"].append(
                    sub_DEBUG_extend_mask[0].cpu()
                )

                debug_outputs["DEBUG_context_from_mask"].append(
                    get_debug_crop_cpu(sub_mask, p_co).squeeze(0)
                )
                debug_outputs["DEBUG_context_from_mask_location"].append(
                    processor.debug_context_location_in_image(sub_image, *p_co)
                    .squeeze(0)
                    .cpu()
                )
                debug_outputs["DEBUG_context_expand"].append(
                    get_debug_crop_cpu(sub_mask, p_cg).squeeze(0)
                )
                debug_outputs["DEBUG_context_expand_location"].append(
                    processor.debug_context_location_in_image(sub_image, *p_cg)
                    .squeeze(0)
                    .cpu()
                )
                debug_outputs["DEBUG_context_with_context_mask"].append(
                    get_debug_crop_cpu(sub_mask, p_cc).squeeze(0)
                )
                debug_outputs["DEBUG_context_with_context_mask_location"].append(
                    processor.debug_context_location_in_image(sub_image, *p_cc)
                    .squeeze(0)
                    .cpu()
                )

                debug_outputs["DEBUG_context_to_target"].append(p_mask.squeeze(0).cpu())
                debug_outputs["DEBUG_context_to_target_location"].append(
                    processor.debug_context_location_in_image(sub_image, *p_cf)
                    .squeeze(0)
                    .cpu()
                )
                debug_outputs["DEBUG_context_to_target_image"].append(
                    p_crop.squeeze(0).cpu()
                )
                debug_outputs["DEBUG_context_to_target_mask"].append(
                    p_mask.squeeze(0).cpu()
                )
                debug_outputs["DEBUG_canvas_image"].append(
                    canvas_image.squeeze(0).cpu()
                )
                debug_outputs["DEBUG_orig_in_canvas_location"].append(
                    processor.debug_context_location_in_image(
                        canvas_image, cto_x, cto_y, cto_w, cto_h
                    )
                    .squeeze(0)
                    .cpu()
                )
                debug_outputs["DEBUG_cropped_in_canvas_location"].append(
                    processor.debug_context_location_in_image(
                ...
                debug_outputs["DEBUG_cropped_mask_blend"].append(
                    p_mask_blend.squeeze(0).cpu()
                )
                ...
                if name.startswith("DEBUG_"):
                    ...
                    if not values:
                        ...
                        if name.endswith("_image") or name.endswith("_location"):
                            final_debug_outputs.append(
                                torch.zeros((count, 1, 1, 3), device="cpu")
                            )
                        else:
                            final_debug_outputs.append(
                                torch.zeros((count, 1, 1), device="cpu")
                            )
                    else:
                        try:
                            ...
                            print(
                                f"InpaintCropImproved: Failed to stack {name}. Error: {e}"
                            )
                            count = result_image.shape[0]
                            if name.endswith("_image") or name.endswith("_location"):
                                final_debug_outputs.append(
...
class InpaintStitchImproved:
    ...
    CATEGORY = "inpaint"
    DESCRIPTION = (
        "Stitches an image cropped with Inpaint Crop back into the original image"
    )
    ...
    def inpaint_stitch(self, stitcher, inpainted_image):
        ...
        for key in ["canvas_image", "cropped_mask_for_blend"]:
            if key in stitcher:
                stitcher[key] = [
                    t.to(device) if torch.is_tensor(t) else t for t in stitcher[key]
                ]
        ...
        for i in range(batch_size):
            ...
            (one_image,) = self.inpaint_stitch_single_image(
                one_stitcher, one_image, processor
            )
    ...
    def inpaint_stitch_single_image(self, stitcher, inpainted_image, processor):
        ...
        # Get rotation angle and apply inverse rotation to inpainted image
        rotation_angle = stitcher.get("rotation_angle", 0)
        if rotation_angle != 0:
            # Inverse rotation: rotate back by (360 - angle)
            inverse_angle = (360 - rotation_angle) % 360
            inpainted_image = processor.rotate_image(inpainted_image, inverse_angle)
            mask = processor.rotate_mask(mask, inverse_angle)

        output_image = processor.stitch_magic_im(
            canvas_image,
            inpainted_image,
            mask,
            ctc_x,
